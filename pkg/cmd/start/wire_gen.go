...
func newInjector(contextContext context.Context, cacheConfig *cache.Config, configConfig *config.Config, commonConfig *common.Config, logger *log.Logger) (*injector, func(), error) {
	...
	fsStore, cleanup2, err := fs.ProvideStore(contextContext, store, configConfig, pool, logger)
	if err != nil {
		cleanup()
		return nil, nil, err
	}
	...
	vHostMap, cleanup3, err := common.ProvideVHostMap(contextContext, logger, vhostServer)
	if err != nil {
		cleanup2()
		cleanup()
		return nil, nil, err
	...
	serverServer, cleanup4, err := server.ProvideServer(contextContext, busyLatch, v, commonConfig, logger, mux, tokenServer)
	if err != nil {
		cleanup3()
		cleanup2()
		cleanup()
