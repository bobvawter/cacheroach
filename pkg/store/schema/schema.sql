...
(
    ...
    chunk  BYTEA NOT NULL CHECK (length(chunk) > 0),
...
(
    ...
    hash         BYTEA CHECK (hash IS NULL OR length(hash) > 0),
...
(
    ...
    hash    BYTEA       NOT NULL CHECK (length(hash) > 0),
...
(
    principal      UUID        NOT NULL PRIMARY KEY,
    ...
    -- The time at which the claims must be revalidated
    refresh_after  TIMESTAMPTZ NOT NULL DEFAULT 0::TIMESTAMPTZ,
    -- Refresh state enum
    refresh_status INT8        NOT NULL DEFAULT 0,
    -- OAuth2 refresh token to achieve revalidation
...
(
    ...
    -- null for principal-only accesses
    tenant       UUID REFERENCES tenants (tenant) ON DELETE CASCADE,
    -- A specific path or prefix wildcard
    path         STRING,
    -- A super-token session
    super        BOOL        NOT NULL DEFAULT false,
    -- a bit-mask field
    capabilities INT8        NOT NULL,
    expires_at   TIMESTAMPTZ NOT NULL,
    issued_at    TIMESTAMPTZ NOT NULL,
    jwt_claims   JSONB,
    note         STRING,
    -- Allows the session to have a well-known name for ease of lookup
    name         STRING,
...
(
    -- This will also be the rope ID.
    upload     UUID        NOT NULL PRIMARY KEY,
